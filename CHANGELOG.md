...
- **密码框切安全键盘卡顿**：登录密码改为普通键盘加圆点遮罩，避免 `InputType.Password` 拉起系统安全键盘；登录页键盘弹出时不再整页上抬重布局，应用市场「点击响应」不再被拖到 600ms+。

### 改进

- **版本号**：`versionName` `1.4.2.7`（`versionCode` 1004011）。
...
> 本版本仅上传未签名包；`.p12` / `.p7b` / `build-profile.json5` 等密钥材料严禁入库或随 Release 分发。已安装 `1.4.2.6` 或更早版本的设备可直接覆盖升级（`versionCode` 1004011）。
...
- **导出诊断日志**：「我的」可导出本机崩溃快照与播放失败等摘要，自行发给开发者；不含密码或完整 cookie，也不会自动上报。

### 修复

- **播放失败后重试闪退**：错误态播放器先复位再重建 surface，避免绑到已失效实例导致闪退。
- **诊断日志导出失败**：目录未真正建成就去读/写时会提示导出失败；现写入应用缓存目录并确认目录可用。

### 改进

- **隐私政策**：说明本机可保存崩溃快照与诊断摘要，由用户主动导出。
- **版本号**：`versionName` `1.4.2.4`（`versionCode` 1004008）。
...
> 本版本仅上传未签名包；`.p12` / `.p7b` / `build-profile.json5` 等密钥材料严禁入库或随 Release 分发。已安装 `1.4.2.2` 或商店更早版本的设备可直接覆盖升级（`versionCode` 1004008）。

---

## [1.4.2.2] - 2026-09-17

在 [1.4.2-beta.1] 上发布。修飞牛 ID 登录后切网缺少凭据，并纳入上架口径、隐私政策与图库删除 / 预览等改动。

### 修复

- **切网凭据**：登录成功即在本机保存切网口令（优先 HUKS，封存失败则明文兜底）。杀进程、换网络后可自动 / 手动切网，不再提示「缺少保存的登录凭据」。勾选「记住密码」只控制退出后是否回填。
- **「我的」页滚动**：退出登录等底部入口不再被底栏挡住。

### 新增

- **永久删除**：相册删除可走彻底删除。
- **文件夹 / 文件删除**：目录树支持删除文件夹与文件。
- **相似照片搜索**：预览可查相似图。

### 改进

- **上架口径**：公开发布包不再声明 `READ_IMAGEVIDEO`，上传走系统选图；`versionName` `1.4.2.2`（`versionCode` 1004006），`vendor` 为皮朝猛。
- **隐私政策**：开发者主体改为华为账号名称「皮朝猛」，补充客服邮箱；「我的」含已收集个人信息清单与第三方共享清单。
- **预览与播放**：下滑退出、详情面板滚动、视频亮度 / 音量手势、幻灯片原图锁定。
- **登录页**：转场与输入跟手；关闭系统密码保险箱自动填充，避免误弹更新提示。
...
> 本版本仅上传未签名包；`.p12` / `.p7b` / `build-profile.json5` 等密钥材料严禁入库或随 Release 分发。已安装 `1.4.2-beta.1` 或商店 `1.4.2.1` 的设备可直接覆盖升级（`versionCode` 1004006）。
...
- **文件夹浏览**：图库底栏文件夹入口进入 NAS 目录树；子目录走 `folder_view/getFolderList`，媒体走 `getFileList`，支持下钻、封面、划选多选、批量下载/删除/幻灯片。选中文件夹时可向下收集媒体（最多 4000 项 / 200 个目录 / 16 层）。
- **跳转到文件夹视图**：时间线与分类「更多」可跳到当前目录；从媒体目录进入时锁定入口层，返回不会越过目标文件夹或落到系统根。
- **搜索选项**：对齐飞牛官方筛选——整理（收藏、无相册/标签/地点）、文件类型、人物（带头像）、拍摄时间/地点、相册、文件夹，以及拍摄信息（机型、焦距、快门、光圈、ISO）。正向条件走 `filters`，「无 xxx」走 `antiFilters`；无关键词也可只按筛选搜索。
- **视频原片边下边播**：HTTP 写入本地缓存，播放器走 `dataSrc`；另拉文件尾（moov）以便半成品即可起播，下完后切本地 `fd://`。公网用小 Range 限速，内网满载不设上限。
- **进度条小窗预览**：拖进度从本地预取抽帧，不再向 NAS 抽帧以免抢下载带宽。
- **满载模式**：「我的」可手动开启；命中可信内网槽位时自动满载，公网/中继默认受控。开关不落盘，杀进程后按当前通道重判。
- **性能档**：按整机内存分高性能 / 均衡 / 流畅，启动与前后台、`onMemoryLevel` 时探测，调节宫格缓存行数等。

### 改进

- **文件夹封面**：有封面时底部渐变遮罩加左下文件夹图标，不再直接铺照片；无封面用独立底色加大图标。
- **自动选路优先内网**：内网可达时不因公网延迟略低而切走；公网域名与 IP 都通则优先域名。
- **上传可选视频**：系统选图 MIME 从仅图片改为图片+视频，单次仍最多 500 个。
- **时间线宫格**：抽出独立行组件，滚动复用不向整屏灌 `@Prop`。
- **缩略图缓存**：同一媒体不同密度的 PixelMap 按 identity 去重，淘汰时不释放仍在屏幕上的实例。
- **划选贴边滚**：速度区间对齐系统相册（约 315～1200 vp/s）。
- **连接探测**：当前连接做成独立卡片，其他路径两行排布延迟。
- **子页顶栏**：分类、人物等改用官方 HdsNavigation 返回按钮。
- **半模态材质**：`bindSheet` 只保留官方 `ULTRA_THICK`，去掉整板触摸流光，避免深色模式跟随手指的大片点光源。

### 修复

- **预览详情路径**：已在同一文件夹打开时点路径只关闭预览回到下层，不再叠一层目录页。
...
> 本版本仅上传未签名包；`.p12` / `.p7b` / `build-profile.json5` 等密钥材料严禁入库或随 Release 分发。已安装 `1.4.1-beta.1` 或 `1.4.0` 的设备可直接覆盖升级（`versionCode` 1004003）。

---

## [1.4.1-beta.1] - 2026-09-09

在 [1.4.0] 上补齐文件夹浏览与搜索筛选，并改进视频原片播放、内网选路与性能档。
...
> **Beta 预发布**：仅上传未签名包，供测试验证。已安装 `1.4.0` 的设备可直接覆盖升级（`versionCode` 1004002）。
